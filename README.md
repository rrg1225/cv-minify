...
- **AI 润色降级**：如果没有配置 `/api/ai-polish` 后端，按钮会使用本地规则生成可展示的优化建议。
- **写作辅助工具**：顶部实时展示字符数、词数、标题数，并支持一键下载 Markdown 源文件。
...
- **AI polish fallback**: when `/api/ai-polish` is not available, the app generates local rule-based suggestions instead of failing.
- **Authoring utilities**: live character, word, and heading counts plus one-click Markdown download.
